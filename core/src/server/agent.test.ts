...
describe('agent session', () => {
  it('createSession with custom TTL', async () => {
    ...
    assert.ok(token);
    const session = await resolveToken(tree, token);
    assert.ok(session);
  ...
  it('createSession with custom claims stores and retrieves them', async () => {
    ...
    const token = await createSession(tree, 'agent:/agents/bot', { claims });
    const session = await resolveToken(tree, token);
    assert.ok(session);
  ...
  it('regular session has no claims field', async () => {
    const token = await createSession(tree, 'alice');
    const session = await resolveToken(tree, token);
    assert.ok(session);
...
describe('agent TOFU flow', () => {
  ...
  it('idle → pending on first connect', async () => {
    ...
    const node = await tree.get(PORT_PATH);
    assert.ok(node);

    // Simulate agentConnect: idle → pending
    await tree.set({ ...node, status: 'pending', pendingKey: keyHash });

    const updated = await tree.get(PORT_PATH);
  ...
  it('approve moves pending → approved, creates user, sets ACL', async () => {
    ...
    // Verify
    const approved = await tree.get(PORT_PATH);
    ...
    const user = await tree.get(`/auth/users/${agentUserId}`);
    assert.ok(user);
  ...
  it('approved + correct key = session with agent userId', async () => {
    ...
    // Create session
    const token = await createSession(tree, agentUserId, { ttlMs: AGENT_SESSION_TTL });
    const session = await resolveToken(tree, token);
    assert.ok(session);
    assert.equal(session.userId, agentUserId);

    // buildClaims includes 'agent' group
    const claims = await buildClaims(tree, agentUserId);
  ...
  it('agent can write to its subtree via ACL', async () => {
    ...
    const aclStore = withAcl(tree, agentUserId, claims);
  ...
  it('revoke clears access', async () => {
    ...
    const revoked = await tree.get(PORT_PATH);
  ...
  it('reset returns to idle', async () => {
    ...
    const reset = await tree.get(PORT_PATH);
